...
WHERE a.weight > 20 AND b.weight > 20
MERGE (p1)<-[r:DrugsCommon]->(p2)
ON CREATE SET r.weight=1
ON MATCH SET r.weight=r.weight+1

MATCH (p1:Pathology)<-[:DrugsCommon]->(p2:Pathology) RETURN p1, p2 LIMIT 20
~~~
...
CREATE INDEX FOR (n:Person) ON (n.code);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/santanche/lab2learn/master/data/faers-2017/drug-use.csv' AS line
MATCH (d:Drug {code: line.codedrug})
MATCH (p:Pathology {code: line.codepathology})
MATCH (pe:Person {code: line.idperson})
CREATE (d)-[:UsadoPor]->(pe)
CREATE (pe)-[:TemDoenca]->(p)

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/santanche/lab2learn/master/data/faers-2017/sideeffect.csv' AS line
MATCH (p:Pathology {code: line.codePathology})
MATCH (pe:Person {code: line.idPerson})
CREATE (pe)-[:EfeitoColateral]->(p)

MATCH (d:Drug)-[:UsadoPor]->(pe:Person)-[:EfeitoColateral]->(p:Pathology)
MERGE (d)-[r:CausaEfeitoColateral]->(p)
ON CREATE SET r.weight = 1
ON MATCH SET r.weight = r.weight + 1;
~~~
